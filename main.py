...
def gregorian_to_jalali(gy, gm, gd):
    """فرمول نهایی و اصلاح‌شده تبدیل تاریخ (بدون باگ اعدادی مثل ۲۳۴)"""
...
def smart_sync_engine():
    """موتور همگام‌سازی تفاضلی (اصلاح باگ تداخل داده‌ها و ثبت تکراری)"""
    ...
    try:
        with db_lock:
            ...
            # امنیت: حذف اطلاعات ورود شخصی قبل از ارسال به هاست
        ...
        # ارسال تغییرات با تایم‌اوت کنترل شده
        r = requests.post(
            IRAN_BRIDGE_URL, 
            json=payload, 
            headers=HEADERS, 
        ...
        if r.status_code == 200 and "OK_SUCCESS" in r.text:
            ...
            # فراخوانی فوری دریافت دیتا برای اعمال حذف‌های دستی در هاست
...
def check_auto_unban():
    """بررسی آزادکردن خودکار با زمان هماهنگ شده ایران (اصلاح باگ پیمایش دیکشنری)"""
    now = get_accurate_now()
    changed = False
    with db_lock:
        if not isinstance(DATA.get("banned_list"), dict): DATA["banned_list"] = {}
        # استفاده از لیست کلیدها برای جلوگیری از RuntimeError در زمان تغییر محتوا
...
def load_db():
    """بارگذاری با رعایت تفکیک تنظیمات محلی و دیتای عمومی"""
        ...
        if r.status_code == 200:
            ...
            if cloud and isinstance(cloud, dict):
                with db_lock:
                    ...
                    DATA["global_notice"] = cloud.get("global_notice", "")
...
class LoginScreen(Screen):
    def on_enter(self): 
        ...
        if u and p and auto:
            ...
            Clock.schedule_once(self.login, 1.0)
    ...
    def _final_login_check(self):
        ...
        if platform == 'android':
            ...
            except: dev_id = socket.gethostname() + "_android_v2"
            ...
            if u in users_db and users_db[u]["pass"] == p:
                ...
                if user_info.get("status") == "approved":
                    ...
                    if user_info["device"] == dev_id:
                        App.get_running_app().session_user = u
                        LOCAL_SETTINGS["saved_creds"] = {"u": u, "p": p, "auto_login": True}
                        save_local_settings(); save_db({"action": "user_login", "u": u}); self.manager.current = 'entry'
                    else: self.u.text = "GOSHI MOJAZ NIST!"
                else: self.u.text = "NAZER TAYID NASHOD"
            else: self.u.text = "USER YA PASS GHALAT"
...
class EntryScreen(Screen):
    ...
    def update_notice_display(self):
        self.notice_lbl.text = DATA.get("global_notice", "نظارت آنلاین فعال است")
    ...
    def select_khalaf(self, name):
        ...
        with db_lock:
            db_copy = DATA.get("game_db", {}).get(uid, {})
    ...
    def submit(self, x):
        ...
        with db_lock:
            ...
            if count >= 10:
                ...
                DATA["banned_list"][uid] = ban_entry; DATA["game_db"][uid][vt] = 0
                self.reason_box.start_ban_blink(vt); save_db({"action": "auto_ban", "uid": uid, "info": ban_entry})
            else: save_db({"action": "report", "uid": uid, "khalaf": vt, "new_count": count})
...
class StatusScreen(Screen):
    ...
    def refresh(self, *args):
        ...
        with db_lock: items = list(DATA.get("game_db", {}).items())
        for uid, reps in items:
            ...
            if active:
                ...
                with card.canvas.before:
                    ...
                    r = RoundedRectangle(pos=card.pos, size=card.size, radius=[12,])
    ...
    def quick_unb(self, uid):
        u = getattr(App.get_running_app(), 'session_user', '')
        if u == "admin" or u in DATA.get("permissions", []) or self.adm_key.text == "MAHDI@#25#":
            with db_lock: DATA.get("game_db", {}).pop(uid, None)
...
class BlacklistScreen(Screen):
    ...
    def refresh(self):
        ...
        with db_lock: bl_list = list(DATA.get("blacklist", []))
        for uid in bl_list:
    ...
    def un_blacklist(self, uid):
        ...
        if u == "admin" or self.key.text == "MAHDI@#25#":
            with db_lock:
                if uid in DATA.get("blacklist", []): DATA["blacklist"].remove(uid)
...
class BannedScreen(Screen):
    ...
    def secure_unb(self, uid):
        u = getattr(App.get_running_app(), 'session_user', '')
        if u == "admin" or self.key.text == "MAHDI@#25#":
            with db_lock: DATA.get("banned_list", {}).pop(uid, None)
...
class AdminPanel(Screen):
    def __init__(self, **kw):
        ...
        bg.add_widget(ModernButton(text="TAYIDE NAZERIN (Requests)", bg_color=(0.1, 0.4, 0.4, 1), on_press=self.show_req_popup))
        bg.add_widget(ModernButton(text="LIST & EKRAJE NAZERIN", bg_color=(0.2, 0.3, 0.5, 1), on_press=self.show_staff_mgmt))
    ...
    def show_req_popup(self, x):
        ...
        with db_lock: pending = list(DATA.get("pending_requests", {}).items())
        for u, p in pending:
    ...
    def approve(self, u, p):
        with db_lock:
            if u in DATA.get("ejected_users", []): DATA["ejected_users"].remove(u)
            DATA["users"][u] = {"pass": p, "status": "approved", "device": ""}; DATA.get("pending_requests", {}).pop(u, None)
        save_db({"action": "approve_user", "u": u, "p": p})

    def show_staff_mgmt(self, x):
        box = BoxLayout(orientation='vertical', padding=10, spacing=10); scroll = ScrollView(); grid = GridLayout(cols=1, spacing=10, size_hint_y=None)
        grid.bind(minimum_height=grid.setter('height')); scroll.add_widget(grid)
        with db_lock: users = list(DATA.get("users", {}).items())
        for u, info in users:
    ...
    def eject(self, u):
        with db_lock:
            ...
            if u not in DATA.get("ejected_users", []): DATA["ejected_users"].append(u)
    ...
    def show_staff_logs(self, x):
        ...
        grid.bind(minimum_height=grid.setter('height')); scroll.add_widget(grid)
        with db_lock: logs = list(DATA.get("staff_activity", []))
        for entry in logs:
    ...
    def show_tools_popup(self, x):
        box = BoxLayout(orientation='vertical', padding=15, spacing=10); not_inp = ModernInput(hint_text="Global Notice..."); not_inp.text = DATA.get("global_notice", "")
        ...
        def save_tools(instance):
            with db_lock:
                ...
                if bl_inp.text and bl_inp.text not in DATA.get("blacklist", []): DATA["blacklist"].append(bl_inp.text)
    ...
    def show_perm_mgmt(self, x):
        box = BoxLayout(orientation='vertical', padding=10, spacing=10); scroll = ScrollView(); grid = GridLayout(cols=1, spacing=10, size_hint_y=None)
        grid.bind(minimum_height=grid.setter('height')); scroll.add_widget(grid)
        with db_lock: staff = list(DATA.get("users", {}).keys())
        for u in staff:
            if u == "admin" or DATA["users"][u].get("status") != "approved": continue
            row = BoxLayout(size_hint_y=None, height=50, spacing=10); is_granted = u in DATA.get("permissions", [])
